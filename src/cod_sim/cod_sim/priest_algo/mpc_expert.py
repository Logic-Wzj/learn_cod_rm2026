...
class batch_crowd_nav():
    ...
    def __init__(self, a_obs, b_obs, v_max, v_min, a_max, num_obs, t_fin, num, num_batch, maxiter, maxiter_cem, weight_smoothness, weight_track, way_point_shape, v_des):
        ...
        self.rho_proj = 1
        self.num_sample = 70  # 控制输出 = 前 num_sample 个采样点平均速度。t_fin=2 时 0.02s/采样，
                              # 70=前1.4s 平均，远目标输出 ~2.1m/s（对齐 MPPI）。
    ...
    def compute_cost_batch(self, x, y, xdot, ydot, xddot, yddot, x_project, y_project, res_norm_batch, x_fin, y_fin,  x_obs_trajectory, y_obs_trajectory):
        ...
        # 路径长度代价：直线最短，惩罚轨迹弧线/绕弯（解决轨迹大弧度弧线问题）
    ...
    def compute_controls(self, c_x_best, c_y_best, delta_t):
        # 用轨迹"前段(0.45s)位移"算速度：跟随轨迹的避障弯。
        # 原"终点位移"是直线到 x_fin，x_fin 在障碍另一侧时会穿过障碍 → 避障失效。
        # 轨迹前段已经开始避障弯，用它作为控制方向让车沿避障轨迹走。
        x_best = jnp.dot(self.P_up_jax, c_x_best)
        y_best = jnp.dot(self.P_up_jax, c_y_best)
        look_ahead = 30  # 前 30/100 采样 = 0.45s (t_fin=1.5)
        dt_look = self.t_fin * look_ahead / 100.0
        vx_control = (x_best[look_ahead] - x_best[0]) / dt_look
        vy_control = (y_best[look_ahead] - y_best[0]) / dt_look
        ax_control = 0.0
        ay_control = 0.0
