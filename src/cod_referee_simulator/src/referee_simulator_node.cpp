// 模拟裁判系统：订阅 /referee/cmd，发布 /referee/game_status + /referee/robot_status
...
public:
  ...
  RefereeSimulatorNode() : Node("referee_simulator_node")
  {
    ...
    this->declare_parameter<int32_t>("max_ammo", 100);
    this->declare_parameter<int32_t>("heat_limit", 240);
    ...
    game_status_pub_ = this->create_publisher<cod_referee_interfaces::msg::GameStatus>(
      "/referee/game_status", 10);
    robot_status_pub_ = this->create_publisher<cod_referee_interfaces::msg::RobotStatus>(
...
private:
  ...
  void publish_status()
  {
    auto gs = cod_referee_interfaces::msg::GameStatus();
    gs.stage = stage_;
    gs.stage_remain_time = stage_time_left_;
    game_status_pub_->publish(gs);

    auto rs = cod_referee_interfaces::msg::RobotStatus();
  ...
  rclcpp::Publisher<cod_referee_interfaces::msg::GameStatus>::SharedPtr game_status_pub_;
  rclcpp::Publisher<cod_referee_interfaces::msg::RobotStatus>::SharedPtr robot_status_pub_;
